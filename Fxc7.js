...
const vcard = 'BEGIN:VCARD\n'
            + 'VERSION:3.0\n'
            + 'FN:𝗞𝗨𝗣𝗥𝗜𝗧\n'
            + 'ORG:Owner 𝗞𝗨𝗣𝗥𝗜𝗧 𝗕𝗢𝗧;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6285758779973:+62 857-5877-9973\n'
            + 'END:VCARD'

prefix = "!"
name = "~ 𝗞𝗨𝗣𝗥𝗜𝗧 𝗕𝗢𝗧"
rdaftar = "TERIMA KASIH TELAH DAFTAR MENJADI TEMEN 𝗞𝗨𝗣𝗥𝗜𝗧 𝗕𝗢𝗧😁"
rmenu = "HAI TEMEN 𝗞𝗨𝗣𝗥𝗜𝗧 𝗕𝗢𝗧👋 JANGAN LUPA DONASI YAA:)"
botinfo = "UNTUK INVITE BOT SILAHKAN DONASI DULU YAA:)"
limitt = 10
memberLimit = 2
ban = []
premium = ["6285758779973@s.whatsapp.net"]
...
async function starts() {
	...
	frhan.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Akhir Nya Beban Berkurang @${num.split('@')[0]} Semoga Tenang Di Alam Sana Di Temani Kakek Sugiono :V`
				let buff = await getBuffer(ppimg)
				frhan.sendMessage(mdata.id, buff, MessageType.image, {caption: teks, contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	frhan.on('chat-update', async (mek) => {
		try {
			...
			const ownerNumber = ["6285758779973@s.whatsapp.net"] // owner number ubah aja
		...
		case 'daftar':
         ...
         case 'lirik':
                ...
                if (!isPublic) return reply(mess.only.publikG)
                    ...
                    frhan.sendMessage('6285758779973@s.whatsapp.net', options, text, {quoted: mek})
...
//akhir kreator
		    ...
		    case 'map':
			    ...
			    if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔═══✪ Tag By *${pushname2}* ✪══`+ teks +'╚═══〘 𝗞𝗨𝗣𝗥𝗜𝗧 𝗕𝗢𝗧 〙═══', members_id, true)
					break
			    case 'mentionall':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
			    if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 𝗞𝗨𝗣𝗥𝗜𝗧 𝗕𝗢𝗧 〙', members_id, true)
					break
			    case 'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
